...
export class BasicInformationServer extends Base {
  ...
  private update(entity: HomeAssistantEntityInformation) {
    ...
    const nodeLabel =
      ...
      ellipse(32, toStringValue(attributes.friendly_name)) ??
      ellipse(32, entity.entity_id);

    const vendorName = resolveVendorName(
      device,
      attributes,
      mapping?.customVendorName,
    ...
    const relatedSoftwareVersion = this.resolveRelatedSoftwareVersionString(entity);
...
function resolveProductName(
  ...
  vendorName: string,
  customOverride: string | undefined,
  productNameFromNodeLabel: string | undefined,
  fallback: string,
): string {
  if (customOverride != null && customOverride.trim().length > 0) {
    return ellipse(32, customOverride) ?? hash(32, fallback);
  }
  ...
  const preferredName =
    modelName == null
      ? humanName ?? productNameFromNodeLabel
...
function ellipse(maxLength: number, value?: string): string | undefined {
